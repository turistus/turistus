...
define('ACCESS', true);
ob_start();
//ID do ponto turistico
$id = filter_input(INPUT_GET, "id", FILTER_SANITIZE_NUMBER_INT);
$idGuia = filter_input(INPUT_GET, "idGuia", FILTER_SANITIZE_NUMBER_INT);
//$VGuia = filter_input(INPUT_GET, "VGuia", FILTER_SANITIZE_NUMBER_INT);
echo $id ." ";
echo $idGuia;
...
?>
        <?php
                ...
                idGuia,
                pontosturisticos.name AS nome,
                servicos.valor AS price,
                servicos.nome as nomeGuia
                        FROM pontosturisticos INNER JOIN
                        servicos ON servicos.id = idGuia
                        WHERE pontosturisticos.id =:id LIMIT 1";

                $result_products = $conn->prepare($query);

                // $receber_dados_por = mysqli_fetch_array($result_products)
                // dai posso usar o $receber_dados_por[id] $receber_dados_por[valor]
                $result_products->bindParam(':id', $id, PDO::PARAM_INT);
                $result_products->execute();
                //se a Cont ROW for igual 0 entao nao puxou ID do PT
                if ($result_products->rowCount() == 0) {
                    header("Location: view-products.php?id=$id");
                    die("Erro: página encontrada!<br>");
                }
...
<html lang="pt-br">
    <head>
        ...
        <title>Turist Us - formulario de chekout</title>
    </head>
    <body>
        ...
        if (isset($data['BtnPicPay'])) {
            ...
            if (!$empty_input) {
                ...
                $query_pay_picpay = "INSERT INTO payments_picpays (first_name, last_name, cpf, phone, email, expires_at, product_id, payments_statu_Id, created, guiaId, dataagendada) VALUES (:first_name, :last_name, :cpf, :phone, :email, :expires_at, :product_id, 1, :created, :guiaId, :dataagendada)";
                ...
                //Aqui nesse if o VALUE deve ser o VALOR qe vai PRO QRCODE gerado pelo PICPAY.
                ...
                if ($add_pay_picpay->rowCount()) {
                    ...
                    $data_buy = [
                        ...
                        "value" => (double) $price,//aqui vai o VALOR do Guia pela info do ponto
                        "expiresAt" => $due_date,
                        "buyer" => [
        ...
        <div class="container">
            <div class="py-5 text-center">
                ...
                <img class="d-block mx-auto mb-4" src="../images/logo/LG.jpg" alt="" width="72" height="72">
                    ...
                    <p class="lead" style="color: grey;">Realizando o pagamento para sua diversão.</p>
            </div>
            ...
            <div class="row mb-5">
                ...
                <div class="col-md-4">
                    <p>Valor Total</p>
                    <div class="mb-1 text-muted"> R$ <?php echo number_format($price, 2, ",", "."); ?></div>
                    <div class="mb-1 text-muted"><?php echo $row_product['nomeGuia']; ?></div>
                    ...
                    ?>" >
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="cpf" id="cpf" class="form-control" placeholder="O mesmo CPF cadastrado" maxlength="14" oninput="maskCPF(this)" value="<?php
                                if (isset($data['cpf'])) {
                                    echo $data['cpf'];
                                }
                    ?>" >
